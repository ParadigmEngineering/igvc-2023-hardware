...
// Flag to send a packet, flipped by ISR
bool sendEstop = false;
...
void IRAM_ATTR estopPressedISR () {
  ...
  if (millis() - lastInterruptTime > DEBOUNCE_TIME) {
    // set a flag, we can't go sending a packet in an ISR
    // that would be unseemly
    sendEstop = true;
  }

  // If removing debouncing, just uncomment this line and delete the rest of the ISR
  // sendEstop = true;
}
...
void setup () {
  ...
  attachInterrupt(ESTOP_PIN, estopPressedISR, RISING);
